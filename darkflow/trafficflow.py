...
signals = []
noOfSignals = 4
simTime = 250     # change this to change time of simulation
timeElapsed = 0
...
vehicleCountCoods = [(300,210),(880,210),(880,550),(300,550)]
vehicleOnRoadCoods = [(200,100),(800,100),(800,670),(200,670)]
vehicleCountTexts = ["0", "0", "0", "0"]
vehiclegen=[0,0,0,0]
vehicleonroad=[0,0,0,0]
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        if(direction=='right'):
            ...
            stops[direction][lane] += temp
        simulation.add(self)
        vehiclegen[direction_number]=vehiclegen[direction_number]+1
...
def generateVehicles():
    ...
    while(True):
        ...
        if(direction_number==5):
            continue
        #if((vehiclegen[direction_number]-vehicles[directionNumbers[direction_number]]['crossed'])>20):
         #   continue
...
class Main:
    ...
    pygame.display.set_caption("Traffic Management")
    ...
    while True:
        ...
        for i in range(0,noOfSignals):  
            ...
            vehicleCountTexts[i] = font.render(("Vehicle Crossed: "+str(displayText)), True, black, white)
            screen.blit(vehicleCountTexts[i],vehicleCountCoods[i])
            vehicleonroad[i] = font.render(("Vehicles on Road (Using YOLOv5) : " +str(vehiclegen[i]-int(displayText))), True, black, white)
            screen.blit(vehicleonroad[i],vehicleOnRoadCoods[i])
